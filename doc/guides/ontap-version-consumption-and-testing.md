...
- **VSA Image**: `x-9-17-1p1-gcnv` → `x-9-18-1` (new default)
- **Mediator Image**: `cvo-mediator-x-9-17-1p1` → `cvo-mediator-x-9-18-1` (new default)
...
{
  "images": {
    "vsa_image_name": "x-9-18-1",
    "mediator_image_name": "cvo-mediator-x-9-18-1"
  }
}
```
...
```yaml
workerConfig:
  vsaImageName: "x-9-18-1"
  vsaMediatorImageName: "cvo-mediator-x-9-18-1"
...
```go
assert.Equal(t, "x-9-18-1", req.VLMConfig.Deployment.Images.VSAImageName, "...")
assert.Equal(t, "cvo-mediator-x-9-18-1", req.VLMConfig.Deployment.Images.MediatorImageName, "...")
```
