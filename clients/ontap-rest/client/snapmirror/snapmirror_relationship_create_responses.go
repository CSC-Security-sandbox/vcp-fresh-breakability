...
| 6621991 | CIFS SMB1 option is enabled on the SVM that hosts the SnapMirror Synchronous volume. SnapMirror Synchronous relationships are not supported for volumes hosted on SVMs with SMB1. |
...
| 13304077 | Specified property is only supported for creating a destination endpoint for FabricLink SnapMirror relationships. |
...
| 13304110 | Policy has \"snapshot_lock_mode\" set to \"compliance\" or \"enterprise\" and the cloud provider associated with this object store relationship requires an effective cluster version of ONTAP 9.12.1 or later to support snapshot locking. |
...
| 13304123 | SnapMirror object store endpoint name for a FlexVol volume relationship exceeds the character limit. |
| 13304124 | SnapMirror object store endpoint name for a FlexGroup volume relationship exceeds the character limit. |
...
##### Also for private error msg, add private tags as  and
#####  after adding the error msg at correct place(in sorted order).
