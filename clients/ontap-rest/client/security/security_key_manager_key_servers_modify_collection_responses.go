...
func (o *SecurityKeyManagerKeyServersModifyCollectionDefault) Error() string {
	return fmt.Sprintf("[PATCH /security/key-managers/{uuid}/key-servers][%d] security_key_manager_key_servers_modify_collection default  %+v", o._statusCode, o.Payload)
}
		...
		if o.NodeStates[i] != nil {
			if err := o.NodeStates[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("info" + "." + "connectivity" + "." + "node_states" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
