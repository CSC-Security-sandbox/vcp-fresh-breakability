...
func (o *EmsFilterRuleModifyCollectionDefault) Error() string {
	return fmt.Sprintf("[PATCH /support/ems/filters/{name}/rules][%d] ems_filter_rule_modify_collection default  %+v", o._statusCode, o.Payload)
}
	...
	if o.MessageCriteria != nil {
		if err := o.MessageCriteria.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				...
				return ce.ValidateName("info" + "." + "message_criteria")
			}
			return err
		}
	}
		...
		if o.ParameterCriteria[i] != nil {
			if err := o.ParameterCriteria[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("info" + "." + "parameter_criteria" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
