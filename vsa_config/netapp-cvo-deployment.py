...
def build_address_resource(context, resource_name, resource_type, description, purpose, subnet_project, subnet):
    address_resource = {
        'name': resource_name,
        'type': resource_type,
        'properties': {
...
def build_forwarding_rule_resource(context, resource_name, resource_type, description, vpc_project, vpc_name,
                                   subnet, address_resource, protocol, backend_service_resource, enable_global_access=False):
    forwarding_rule_resource = {
        ...
        'properties': {
            ...
            'loadBalancingScheme': 'INTERNAL',
            'IPAddress': '$(ref.' + address_resource + '.address)',
            'IPProtocol': protocol,
...
def generate_config(context):
    ...
    if ha_deployment_obj:
        ...
        vm1_name = separator.join([deployment_name, 'vm1'])
    ...
    if sole_tenant_node_obj:
        ...
        if not sole_tenant_node_group_name:
            ...
            sole_tenant_node_template_name = separator.join([deployment_name,node_template])
    ...
    if deployment_type == 'singlenode':
        ...
        vm2_name = separator.join([deployment_name, 'vm2'])
        ...
        vm1_nas_interface = separator.join([nas_interface, 'vm1'])
        vm2_nas_interface = separator.join([nas_interface, 'vm2'])

        ### creating ilb names
        ilb_counter = 1
        ilb_names['cluster_mgmt_interface'] = separator.join([deployment_name, 'ilb'+str(ilb_counter), cluster_mgmt_interface])
        ilb_counter+=1
        ...
        if deployment_type == 'non_shared_ha':
            ...
            if mediator_ip:
                ...
                mediator_name = separator.join([deployment_name, 'mediator'])
        ...
        resource_name = instance_group_names['instancegroup1'] = separator.join([deployment_name, "instancegroup1"])
        resources.append(build_instance_group_resource(resource_name, compute_instance_group_v1, description, vm1_zone))

        resource_name = instance_group_names['instancegroup2'] = separator.join([deployment_name, "instancegroup2"])
        ...
        resources.append(
            build_forwarding_rule_resource(context, resource_name, compute_forwarding_rule_v1, description, vnic_vpc_project_for_resources, vnic_network_obj_for_resources,
                                           vnic_subnet_obj_for_resources, vm1_nas_ip_name, tcp, tcp_backend_service_names['vm1_nas_interface'])
        )
        ...
        if deployment_type == 'shared_ha':
            ...
            resource_name = vm2_disk_names[4] = separator.join([vm2_name, 'disk', 'data1copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[5] = separator.join([vm2_name, 'disk', 'data2copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[6] = separator.join([vm2_name, 'disk', 'data3copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[7] = separator.join([vm2_name, 'disk', 'data4copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))
        else:
            # deployment_type is 'non_shared_ha'
            ...
            resource_name = vm2_disk_names[5] = separator.join([vm2_name, 'disk', 'data1copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[6] = separator.join([vm2_name, 'disk', 'data2copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[7] = separator.join([vm2_name, 'disk', 'data3copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

            resource_name = vm2_disk_names[8] = separator.join([vm2_name, 'disk', 'data4copy'])
            resources.append(build_pd_resource(context, resource_name, compute_disk_v1, labels, pd_ssd, data_disk_size_gb, vm1_zone))

        # create a placement policy
        if placement_policy_vm_count:
            resource_name = placement_policy_name = separator.join([deployment_name, "vm", "placementpolicy1", "spread"])
            resources.append({
                'name': resource_name,
        ...
        if deployment_type == 'non_shared_ha':
            ...
            common_customdata = common_customdata + '\niscsi_mediator_ip=' + mediator_ip_address + '\niscsi_mediator_target=' + mediator_obj['mediatorTarget']
