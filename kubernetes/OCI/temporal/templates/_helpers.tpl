...
{{- define "temporal.primaryImageRegistryFullPath" -}}
...
{{- if and (ne $primaryRegistry "") (ne $primaryRegistryPath "") -}}
{{- printf "%s/%s" $primaryRegistry $primaryRegistryPath -}}
{{- else -}}
{{- $primaryRegistry -}}
{{- end -}}
{{- end -}}

{{- define "temporal.secondaryImageRegistryFullPath" -}}
{{- $global := .Values.global | default dict -}}
...
{{- if and (ne $secondaryRegistry "") (ne $secondaryRegistryPath "") -}}
{{- printf "%s/%s" $secondaryRegistry $secondaryRegistryPath -}}
{{- else -}}
{{- $secondaryRegistry -}}
{{- end -}}
{{- end -}}

{{- define "temporal.imageRepository" -}}
{{- $context := index . 0 -}}
{{- $repository := index . 1 -}}
{{- $global := $context.Values.global | default dict -}}
{{- $useSecondary := (get $global "useSecondaryImageRegistry") | default false -}}
{{- if not $useSecondary -}}
{{- $repository -}}
{{- else -}}
{{- $primaryFull := include "temporal.primaryImageRegistryFullPath" $context | trim -}}
{{- $secondaryFull := include "temporal.secondaryImageRegistryFullPath" $context | trim -}}
{{- if or (eq $secondaryFull "") (eq $primaryFull "") -}}
{{- $repository -}}
{{- else if eq $repository $primaryFull -}}
{{- $secondaryFull -}}
{{- else if hasPrefix (printf "%s/" $primaryFull) $repository -}}
{{- printf "%s/%s" $secondaryFull (trimPrefix (printf "%s/" $primaryFull) $repository) -}}
{{- else -}}
{{- $repository -}}
{{- end -}}
{{- end -}}
