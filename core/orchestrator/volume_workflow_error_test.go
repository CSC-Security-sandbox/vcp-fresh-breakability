...
func TestGetVolume_JobUpdateOnWorkflowFailure(t *testing.T) {
	t.Run("ShouldMarkJobAsErrorWhenRefreshWorkflowFails", func(tt *testing.T) {
		ctx := context.WithValue(context.Background(), middleware.TemporalSLoggerKey, log.Fields{"key": "value"})
		mockStorage := database.NewMockStorage(tt)
		mockTemporal := workflowEngineMock.NewMockTemporalTestClient(tt)
		...
		mockStorage.On("DescribeVolume", ctx, "volume-uuid").Return(volume, nil)
		mockStorage.On("GetJobsWithCondition", ctx, mock.Anything).Return([]*datamodel.Job{}, nil)
		mockStorage.On("CreateJob", ctx, mock.AnythingOfType("*datamodel.Job")).Return(job, nil)

		// Mock workflow failure
		workflowErr := errors.New("workflow execution failed")
		mockTemporal.EXPECT().ExecuteWorkflow(ctx, mock.Anything, mock.Anything, mock.Anything).Return(nil, workflowErr)
		...
		// Execute test
		result, err := orch.GetVolume(ctx, "volume-uuid", true)
...
// Test for line 488: Failed to update job state to ERROR during refresh workflow
func TestRefreshVolume_FailedJobUpdateOnError(t *testing.T) {
	t.Run("ShouldLogErrorWhenJobUpdateFailsDuringRefreshErrorHandling", func(tt *testing.T) {
		ctx := context.WithValue(context.Background(), middleware.TemporalSLoggerKey, log.Fields{"key": "value"})
		mockStorage := database.NewMockStorage(tt)
		mockTemporal := workflowEngineMock.NewMockTemporalTestClient(tt)
		...
		// Mock UpdateJob call to fail - this is what triggers line 488
		jobUpdateErr := errors.New("failed to update job")
		mockStorage.On("UpdateJob", ctx, job.UUID, string(models.JobsStateERROR), 0, workflowErr.Error()).Return(jobUpdateErr)
