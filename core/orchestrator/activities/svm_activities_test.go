...
// ---------------------------------------------------------------------------
// GetSvmAdminOntapPasswordSecretForOCI
// ---------------------------------------------------------------------------
...
func TestGetSvmAdminOntapPasswordSecretForOCI(t *testing.T) {
	...
	t.Run("nil svm returns non-retryable error", func(t *testing.T) {
		...
		_, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, (*datamodel.Svm)(nil), pool)

		assert.Error(t, err)
		assert.Contains(t, err.Error(), "svm must not be nil")
	})

	t.Run("nil pool returns non-retryable error", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)

		_, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, (*datamodel.Pool)(nil))

		assert.Error(t, err)
		assert.Contains(t, err.Error(), "pool must not be nil")
	})

	t.Run("nil pool credentials returns non-retryable error", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)

		poolWithoutCredentials := *pool
		poolWithoutCredentials.PoolCredentials = nil
		_, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, &poolWithoutCredentials)

		assert.Error(t, err)
		assert.Contains(t, err.Error(), "pool.PoolCredentials must not be nil")
	})

	t.Run("default auth returns pool password", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)
		...
		encodedValue, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, pool)
		require.NoError(t, err)
		var creds vlm.OntapCredentials
		require.NoError(t, encodedValue.Get(&creds))
		assert.Equal(t, "pool-admin-pw", creds.AdminPassword)
	})

	t.Run("USER_CERTIFICATE auth is rejected", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)

		certPool := *pool
		certPool.PoolCredentials = &datamodel.PoolCredentials{AuthType: env.USER_CERTIFICATE}
		_, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, &certPool)

		assert.Error(t, err)
		assert.Contains(t, err.Error(), "USER_CERTIFICATE auth type is not supported")
	})

	t.Run("USERNAME_PWD_SEC_MGR fetches secret by derived name", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)

		encodedSecret := base64.StdEncoding.EncodeToString([]byte("vault-admin-pw"))
		mockSvc := newMockOCIServiceForTest(t, func(req *http.Request) (*http.Response, error) {
			assert.Contains(t, req.URL.String(), "test-deployment")
			return ociMockJSONResponse(http.StatusOK, `{
				"secretId":      "ocid1.vaultsecret.oc1..ontapadmin",
				"versionNumber": 1,
				"secretBundleContent": {
					"contentType": "BASE64",
					"content":     "`+encodedSecret+`"
				}
			}`), nil
		})
		hyperscaler2.GetOCIService = func(ctx context.Context) (*oci.OciServices, error) {
		...
		secretPool := *pool
		secretPool.PoolCredentials = &datamodel.PoolCredentials{AuthType: env.USERNAME_PWD_SEC_MGR}
		encodedValue, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, &secretPool)

		require.NoError(t, err)
		var creds vlm.OntapCredentials
		require.NoError(t, encodedValue.Get(&creds))
		assert.Equal(t, "vault-admin-pw", creds.AdminPassword)
	})

	t.Run("USERNAME_PWD_SEC_MGR returns error when OCI service initialization fails", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)
		...
		_, err := testEnv.ExecuteActivity(act.GetSvmAdminOntapPasswordSecretForOCI, svm, &secretPool)

		assert.Error(t, err)
	})

	t.Run("USERNAME_PWD_SEC_MGR returns error when secret is missing", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		testEnv := ts.NewTestActivityEnvironment()
		testEnv.RegisterActivity(act.GetSvmAdminOntapPasswordSecretForOCI)
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "not found in OCI Vault")
	})
}
