...
func TestFinishProjectEventActivity_HardDeleteResourcesInOrder(t *testing.T) {
	...
	t.Run("VerifyDeletionOrderIsRespected", func(t *testing.T) {
		...
		expectedOrder := []string{
			...
			"volumes", "expert_mode_volumes", "cluster_peerings", "backup_vaults", "svms", "pools", "host_groups",
	...
	t.Run("VerifyQueryFiltersAreCorrect", func(t *testing.T) {
		...
		assert.Equal(t, dbQueryAccountName, queryFilters[15]) // accounts (index 15, uses dbQueryAccountName)
		...
		assert.Equal(t, 16, accountIDQueryCount) // All except backups and accounts
	})
}

func TestResourceConfiguration(t *testing.T) {
	t.Run("VerifyResourceCount", func(t *testing.T) {
		assert.Equal(t, 18, len(resourcesToHardDelete), "Should have 18 resources to delete")
	})
