...
func (va VolumeDeleteActivity) CleanupAutoGeneratedVPG(ctx context.Context, vpgID int64, node *models.Node) error {
	...
	if vpg.OntapQosPolicyID != "" {
		...
		if err != nil {
			...
			if err != nil {
				logger.Warn("Failed to delete QoS policy from ONTAP", "policy_id", vpg.OntapQosPolicyID, "error", err)
			} else {
				logger.Info("Deleted QoS policy from ONTAP", "policy_id", vpg.OntapQosPolicyID)
			}
		}
	}
