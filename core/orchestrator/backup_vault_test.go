...
func TestCreateBackupVaultEntryInVCP_Hydration(t *testing.T) {
	ctx := context.Background()
	mockLogger := log.NewLogger()
	ctx = context.WithValue(ctx, middleware.ContextSLoggerKey, mockLogger)
	...
	t.Run("WhenHydrationDisabled_SkipsHydration", func(t *testing.T) {
		...
		mockStorage.On("CreateBackupVaultEntryInVCP", ctx, mock.Anything).Return(createdBackupVault, nil)

		// Act
		result, err := o.CreateBackupVaultEntryInVCP(ctx, backupVault, params)

		// Assert
		assert.NoError(t, err)
		assert.NotNil(t, result)
		assert.Equal(t, createdBackupVault.UUID, result.UUID)
		mockStorage.AssertExpectations(t)
	})

	t.Run("WhenHydrationEnabledAndHydrationSucceeds_ReturnsSuccess", func(t *testing.T) {
		...
		// Mock common.HydrateCreatedBackupVaults
		originalHydrateCreatedBackupVaults := common.HydrateCreatedBackupVaults
		defer func() { common.HydrateCreatedBackupVaults = originalHydrateCreatedBackupVaults }()
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
			return nil
		}
	...
	t.Run("WhenHydrationEnabledAndHydrationFails_ReturnsError", func(t *testing.T) {
		// Override hydrationEnabled to true
		originalHydrationEnabled := hydrationEnabled
		hydrationEnabled = true
		defer func() { hydrationEnabled = originalHydrationEnabled }()
		...
		expectedError := errors.New("failed to hydrate created backup vault to CCFE")
		...
		// Mock common.HydrateCreatedBackupVaults to fail
		originalHydrateCreatedBackupVaults := common.HydrateCreatedBackupVaults
		defer func() { common.HydrateCreatedBackupVaults = originalHydrateCreatedBackupVaults }()
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
	...
	t.Run("WhenGetAccountFails_ReturnsError", func(t *testing.T) {
		// Override hydrationEnabled to false for this test
		originalHydrationEnabled := hydrationEnabled
		hydrationEnabled = false
		defer func() { hydrationEnabled = originalHydrationEnabled }()
		...
		// Mock getOrCreateAccount to fail
	...
	t.Run("WhenCreateBackupVaultEntryInVCPFails_ReturnsError", func(t *testing.T) {
		// Override hydrationEnabled to false for this test
		originalHydrationEnabled := hydrationEnabled
		hydrationEnabled = false
		defer func() { hydrationEnabled = originalHydrationEnabled }()
		...
		expectedError := errors.New("failed to create backup vault entry")
		...
		// Mock CreateBackupVaultEntryInVCP to fail
		mockStorage.On("CreateBackupVaultEntryInVCP", ctx, mock.Anything).Return(nil, expectedError)
...
func TestHydrateCreatedBackupVaults(t *testing.T) {
	...
	defer func() {
		...
		common.HydrateCreatedBackupVaults = originalHydrateCreatedBackupVaults
	}()

	t.Run("WhenAllStepsSucceed_ReturnsNoError", func(t *testing.T) {
		// Mock auth.GenerateCallbackToken
		auth.GenerateCallbackToken = func(ctx context.Context) (string, error) {
			return "mock-token", nil
		}
		...
		// Mock common.HydrateCreatedBackupVaults
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
			assert.Equal(t, backupVault.Name, backupVaultName)
			assert.Equal(t, backupRegionName, location)
			assert.Equal(t, params.OwnerID, projectId)
			assert.Equal(t, "mock-token", token)
			assert.Len(t, requests, 1)
			assert.NotNil(t, requests[0].BackupVault)
			assert.Equal(t, backupVault.Name, requests[0].BackupVault.ResourceId)
			assert.Equal(t, backupVault.UUID, requests[0].BackupVault.BackupVaultId)
			return nil
		}
	...
	t.Run("WhenGenerateCallbackTokenFails_ReturnsError", func(t *testing.T) {
	...
	t.Run("WhenHydrateCreatedBackupVaultsFails_ReturnsError", func(t *testing.T) {
		expectedError := errors.New("failed to hydrate backup vaults")
	...
	t.Run("WhenBackupVaultHasNilBackupRegionName_Panics", func(t *testing.T) {
		backupVaultWithNilRegion := &datamodel.BackupVault{
			BaseModel:        datamodel.BaseModel{UUID: "backup-vault-uuid", ID: 1},
			Name:             "test-backup-vault",
		...
		// Act & Assert - This should panic when trying to dereference nil BackupRegionName
		assert.Panics(t, func() {
			_ = hydrateCreatedBackupVaults(ctx, backupVaultWithNilRegion, params)
		})
	})

	t.Run("WhenConvertToGCPHydrateBackupVaultCreateRequestsCalled_WithCorrectParameters", func(t *testing.T) {
		// Mock auth.GenerateCallbackToken
		auth.GenerateCallbackToken = func(ctx context.Context) (string, error) {
			return "mock-token", nil
		}

		// Mock common.HydrateCreatedBackupVaults to verify the requests
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
			// Verify that ConvertToGCPHydrateBackupVaultCreateRequests was called correctly
			assert.Len(t, requests, 1)
			assert.NotNil(t, requests[0].BackupVault)
			assert.Equal(t, backupVault.Name, requests[0].BackupVault.ResourceId)
			assert.Equal(t, backupVault.UUID, requests[0].BackupVault.BackupVaultId)
			assert.Equal(t, backupVault.BackupVaultType, requests[0].BackupVault.BackupVaultType)
			assert.Equal(t, backupRegionName, requests[0].BackupVault.BackupRegion)
			return nil
		}
	...
	t.Run("WhenMultipleBackupVaultsInRequest_ProcessesCorrectly", func(t *testing.T) {
		// Note: The function only processes one backup vault, but we test the conversion
		// Mock auth.GenerateCallbackToken
		auth.GenerateCallbackToken = func(ctx context.Context) (string, error) {
			return "mock-token", nil
		}
		...
		// Mock common.HydrateCreatedBackupVaults
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
			// Verify only one request is created (function converts single backup vault)
			assert.Len(t, requests, 1)
			return nil
		}
	...
	t.Run("WhenContextHasLogger_LoggerIsRetrieved", func(t *testing.T) {
		// Mock auth.GenerateCallbackToken
		auth.GenerateCallbackToken = func(ctx context.Context) (string, error) {
			return "mock-token", nil
		}

		// Mock common.HydrateCreatedBackupVaults to verify logger is passed
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
	...
	t.Run("WhenTokenIsGenerated_IsPassedToHydrateFunction", func(t *testing.T) {
		expectedToken := "specific-test-token"
		...
		// Mock common.HydrateCreatedBackupVaults to verify token
		common.HydrateCreatedBackupVaults = func(ctx context.Context, logger log.Logger, requests []models.Request, backupVaultName string, location string, projectId string, token string) error {
