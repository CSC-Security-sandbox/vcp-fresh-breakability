...
func TestPerformMountCheckWorkFlow(t *testing.T) {
	t.Run("TestPerformMountCheckWorkFlow", func(t *testing.T) {
		...
		env.RegisterActivity(mountJobActivity.UpdateVolumeDetailsInDB)
	...
	t.Run("TestPerformMountCheckWorkFlowForFiles", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		env := ts.NewTestWorkflowEnvironment()
		env.SetContextPropagators([]workflow.ContextPropagator{util.NewContextMapPropagator()})
		encodedValue, _ := converter.GetDefaultDataConverter().ToPayload(log.Fields{})
		...
		env.RegisterActivity(mountJobActivity.GetReplicationFromOntap)
		env.RegisterActivity(mountJobActivity.UpdateReplicationInDB)
		env.RegisterActivity(mountJobActivity.MountVolume)
		env.RegisterActivity(mountJobActivity.UpdateVolumeDetailsInDB)
		...
		volume := &datamodel.Volume{
			...
			Svm:              &datamodel.Svm{Name: "svm_test"},
			VolumeAttributes: &datamodel.VolumeAttributes{FileProperties: &datamodel.FileProperties{JunctionPath: "/junctionPath"}},
			PoolID:           1,
		}
		dbreplication := &datamodel.VolumeReplication{ReplicationAttributes: &datamodel.ReplicationDetails{
			ExternalUUID: "external",
	...
	t.Run("TestPerformMountCheckWorkFlowWithMultipleLuns", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		env := ts.NewTestWorkflowEnvironment()
		env.SetContextPropagators([]workflow.ContextPropagator{util.NewContextMapPropagator()})
		encodedValue, _ := converter.GetDefaultDataConverter().ToPayload(log.Fields{})
		...
		env.RegisterActivity(mountJobActivity.GetLunDetailsFromOntap)
		env.RegisterActivity(mountJobActivity.UpdateVolumeDetailsInDB)
		env.RegisterActivity(mountJobActivity.AbortVolumeReplicationForMount)
		env.RegisterActivity(mountJobActivity.BreakVolumeReplicationForMount)
		...
		// Multiple LUNs to trigger the abort/break flow
		multipleLuns := []*vsa.LunResponse{
			{SerialNumber: "lun1"},
			{SerialNumber: "lun2"},
		}
		...
		env.OnActivity(mountJobActivity.GetReplicationFromOntap, mock.Anything, dbreplication, mock.Anything, accountName).Return(dbreplication, nil)
		env.OnActivity(mountJobActivity.UpdateReplicationInDB, mock.Anything, dbreplication, multipleLuns).Return(nil)
		env.OnActivity(mountJobActivity.UpdateVolumeDetailsInDB, mock.Anything, mock.Anything, mock.Anything).Return(nil)
	...
	t.Run("TestPerformMountCheckWorkFlowBreakVolumeReplicationError", func(t *testing.T) {
		var ts testsuite.WorkflowTestSuite
		env := ts.NewTestWorkflowEnvironment()
		env.SetContextPropagators([]workflow.ContextPropagator{util.NewContextMapPropagator()})
		encodedValue, _ := converter.GetDefaultDataConverter().ToPayload(log.Fields{})
		...
		// Assert workflow execution - workflow completes but status is updated to ERROR
		// The workflow returns nil because UpdateJobStatus succeeds, even though Run returns an error
		assert.True(t, env.IsWorkflowCompleted())
		assert.Nil(t, env.GetWorkflowError())
		mockStorage.AssertExpectations(t)
	})
