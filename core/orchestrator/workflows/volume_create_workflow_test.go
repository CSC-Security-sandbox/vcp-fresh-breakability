...
func (s *UnitTestSuite) Test_CreateVolumeWorkflow_RestoreSnapshotWithThickCloneType_Success() {
	// Test the new code path: isRestoreSnapshot && CloneType == "THICK"
	mockStorage := database.NewMockStorage(s.T())
	commonActivity := activities.CommonActivities{SE: mockStorage}
	volumeCreateActivity := activities.VolumeCreateActivity{SE: mockStorage}
	volumeDeleteActivity := activities.VolumeDeleteActivity{SE: mockStorage}
	...
	// This is the key activity that should be called for THICK clone type
	s.env.OnActivity(volumeCreateActivity.InitiateSplitForVolume, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
...
func (s *UnitTestSuite) Test_CreateVolumeWorkflow_RestoreSnapshotWithThickCloneType_InitiateSplitError() {
	...
	// This is the key activity that should fail for THICK clone type
	s.env.OnActivity(volumeCreateActivity.InitiateSplitForVolume, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(errors.New("failed to initiate split for volume"))
	...
	assert.True(s.T(), s.env.IsWorkflowCompleted())
	assert.NotNil(s.T(), s.env.GetWorkflowError())
	assert.Contains(s.T(), s.env.GetWorkflowError().Error(), "failed to initiate split for volume")
}
...
func (s *UnitTestSuite) Test_CreateVolumeWorkflow_WithSplitVolumeAndChildWorkflow() {
	...
	syncBackupZiZsActivity := backgroundactivities.SyncBackupZiZsActivity{SE: mockStorage}
	s.env.OnActivity(syncBackupZiZsActivity.SyncBucketDetails, mock.Anything, mock.Anything).Return(nil, errors.New("tenant project number is required but not found in bucket details"))
...
func (s *UnitTestSuite) Test_UpdateRemoteBackupVaultDetailsInVCP_Success() {
	...
	s.env.OnActivity(volumeCreateActivity.CreateBucket, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(bucketDetails, nil)
...
func (s *UnitTestSuite) Test_UpdateRemoteBackupVaultDetailsInVCP_Error() {
	...
	// Make UpdateRemoteBackupVaultDetailsInVCP fail
	s.env.OnActivity(volumeCreateActivity.UpdateRemoteBackupVaultDetailsInVCP,
		mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(errors.New("failed to update remote backup vault"))
