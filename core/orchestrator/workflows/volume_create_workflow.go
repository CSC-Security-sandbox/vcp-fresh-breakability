...
func (wf *volumeCreateWorkflow) Run(ctx workflow.Context, args ...interface{}) (interface{}, *vsaerrors.CustomError) {
	...
	if createVolumeParams.ThroughputMibps != nil {
		...
		// Create PoolView from dbVolume.Pool for the activity
		...
		err = workflow.ExecuteActivity(ctx, vpgActivity.CreateQoSPolicyInONTAP, vpg, poolView, node).Get(ctx, &qosPolicyID)
		if err != nil {
			log.Errorf("Failed to create QoS policy in ONTAP: %v", err)
			return nil, ConvertToVSAError(err)
		}
		rollbackManager.AddActivity(vpgActivity.DeleteQoSPolicyInONTAP, qosPolicyID, poolView, &node)
	...
	// Create volume in DB (after VPG creation if applicable, so VPG ID is already set)
	// This ensures the volume is created with the VPG ID already populated
	var createdVolume *datamodel.Volume
	err = workflow.ExecuteActivity(dbHbCtx, volumeActivity.CreateVolume, dbVolume).Get(dbHbCtx, &createdVolume)
	if err != nil {
		log.Errorf("Failed to create volume in DB: %v", err)
		return nil, ConvertToVSAError(err)
	}
	// Update dbVolume with the created volume (to get DB-generated fields like ID)
