...
func TestErroredPool(t *testing.T) {
	t.Run("ReturnsErrorWhenPoolDoesNotExist", func(tt *testing.T) {
		...
		_, err = store.ErroredPool(context.Background(), pool, "")
		if err == nil {
			tt.Error("Expected error, got nil")
		}
		// Check that the error is a not found error.
	...
	t.Run("UpdatesPoolStateToErrorSuccessfully", func(tt *testing.T) {
		...
		pool.StateDetails = models.LifeCycleStateCreationErrorDetails

		updatedPool, err := store.ErroredPool(context.Background(), pool, models.LifeCycleStateCreationErrorDetails)
		if err != nil {
			tt.Errorf("Expected no error, got %v", err)
		}
		...
		if dbPool.State != models.LifeCycleStateError {
			tt.Errorf("Expected state %v, got %v", models.LifeCycleStateError, dbPool.State)
		}
		if dbPool.StateDetails != models.LifeCycleStateCreationErrorDetails {
			tt.Errorf("Expected state details %v, got %v", models.LifeCycleStateCreationErrorDetails, dbPool.StateDetails)
		}
