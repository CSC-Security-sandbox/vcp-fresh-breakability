...
func TestVolumeAttributes_Value(t *testing.T) {
	...
	val, err := va.Value()
	assert.NoError(t, err)
	assert.Equal(t, `{"creation_token":"token","protocols":null,"vendor_subnet_id":"","external_uuid":"","block_properties":null,"is_data_protection":false,"labels":null}`, string(val.([]byte)))
}
...
func TestDataProtection_Value(t *testing.T) {
	...
	val, err := dp.Value()
	assert.NoError(t, err)
	assert.Equal(t, `{"scheduled_backup_enabled":true,"backup_vault_id":"vault123","backup_policy_id":"","backup_chain_bytes":null,"policy_enforced":null}`, string(val.([]byte)))
}
