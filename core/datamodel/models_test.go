...
func TestVolumeAttributes_Value(t *testing.T) {
	...
	val, err := va.Value()
	assert.NoError(t, err)
	assert.Equal(t, `{"creation_token":"token","protocols":null,"vendor_subnet_id":"","external_uuid":"","block_properties":null,"block_devices":null,"file_properties":null,"is_data_protection":false,"snap_reserve":0,"labels":null,"restored_backup_id":"","restored_backup_path":"","latest_logical_backup_size":0}`, string(val.([]byte)))
}
