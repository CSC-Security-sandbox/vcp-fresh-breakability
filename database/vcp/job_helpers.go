...
func GetExistingDeleteJobForDeletingState(ctx context.Context, se Storage, resourceUUID string, deleteJobType models.JobType, logger log.Logger) (existingJobUUID string) {
	existingJob, err := se.GetJobByResourceUUID(ctx, resourceUUID, string(deleteJobType))
	if err == nil && existingJob != nil {
		if existingJob.State != string(models.JobsStateDONE) && existingJob.State != string(models.JobsStateERROR) {
...
func ValidateCorrelationIDForCreatingResource(ctx context.Context, se Storage, resourceUUID string, resourceType string, createJobType models.JobType, deleteJobType models.JobType, logger log.Logger) (existingDeleteJobUUID string, isCleanupDelete bool, err error) {
	...
	if err == nil && existingDeleteJob != nil {
		...
		if existingDeleteJob.State != string(models.JobsStateDONE) && existingDeleteJob.State != string(models.JobsStateERROR) {
