...
`
 ...
 from (
 	select
		...
		sum(case when(measured_type = 'COOL_TIER_DATA_READ_SIZE_RAW') then quantity else 0 end) as hourly_cool_tier_read_bytes,
		sum(case when(measured_type = 'COOL_TIER_DATA_WRITE_SIZE_RAW') then quantity else 0 end) as hourly_cool_tier_write_bytes,
